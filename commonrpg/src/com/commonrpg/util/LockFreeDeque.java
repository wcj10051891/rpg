 ...
 * <ol>
 * <li>contains(Object)</li>            链表不应该用contains，如果需要contains操作请用hash表
 * <li>iterator()</li>                      要锁住再遍历
 * <li>descendingIterator()</li>       这个方法还未实现
 * </ol>
 *
 * @author raja, ganzhi
 *
...
public class LockFreeDeque<E> extends AbstractQueue<E> implements Deque<E> {
    ...
    static class AnchorType<E> {
        ...
        //        .newUpdater(AnchorType.class, "faint");
	...
	public void addFirst(E d) {
		...
		/* Create new deque node and putIfAbsent the data into it */
    ...
    public boolean isEmptyAfterPoll() {
        AnchorType<E> old_anchor;
        AnchorType<E> new_anchor = new AnchorType<E>();
    ...
    public boolean isEmptyBeforeAdd(E d) {
        DequeNode<E> newtop;
        AnchorType<E> old_anchor;
        /* Create a new anchor to replace the old one */
        AnchorType<E> update_anchor = new AnchorType<E>(); // new anchor node
    ...
    public List<E> dumpAndClearIfMoreThan(int n) {
        AnchorType<E> old_anchor, new_anchor;
        // Create a new anchor with empty left/right pointers
        new_anchor = new AnchorType<E>(null, null, STABLE, 0);
        ...
        while (true) {
            ...
            // return if already empty
            if (old_anchor.right == null || old_anchor.numElements < n)
                return null;
            ...
            if (anchor.compareAndSet(old_anchor, new_anchor))
            {
                ...
                DequeNode<E> cursor = old_anchor.left;
                while (cursor != null)
                {
