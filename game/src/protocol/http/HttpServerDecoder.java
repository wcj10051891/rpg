...
import protocol.http.api.HttpEndOfContent;
import protocol.http.api.HttpMethod;
...
public class HttpServerDecoder extends Decoder {
	private static final Log log = LogFactory.getLog(HttpServerDecoder.class);
    ...
    public Object decode(ChannelContext session, byte[] data) {
    	...
    	while (msg.hasRemaining()) {
            ...
            case NEW:
                ...
                final HttpRequestImpl rq = parseHttpRequestHead(msg);
            ...
            case BODY:
                ...
                if (chunkSize != 0) {
                	ByteBuffer wb = ByteBuffer.allocate(msg.remaining());
        			wb.put(msg);
        			wb.flip();
        			messageQueue.add(wb);
                }
