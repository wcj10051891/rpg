    ...
    public void messageReceived(ChannelHandlerContext context, MessageEvent event) throws Exception
    {
        ...
        ****/
        ...
        try
        {
            ...
            if (! response.containsHeader(HttpHeaders.Names.CONTENT_TYPE))
                response.setHeader(HttpHeaders.Names.CONTENT_TYPE, "text/plain; charset="+server.defaultCharset);

            //if (!close)
                // There's no need to add 'Content-Length' header
                // if this is the last response.
                if (! response.containsHeader(HttpHeaders.Names.CONTENT_LENGTH))
                    response.setHeader(HttpHeaders.Names.CONTENT_LENGTH, String.valueOf(servletResponse.getContentLength()));
            ...
            if (cookies != null && !cookies.isEmpty())
            {
                for (javax.servlet.http.Cookie cookie : cookies)
                {
                    ...
                    response.addHeader(HttpHeaders.Names.SET_COOKIE, sb.toString());
                }
            }
