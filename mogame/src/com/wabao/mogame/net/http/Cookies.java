package com.wabao.mogame.net.http;
...
public final class Cookies {
	...
	/*
	 * List of Separator Characters (see isSeparator()) Excluding the '/' char
	 * violates the RFC, but it looks like a lot of people putIfAbsent '/' in
	 * unquoted values: '/': ; //47 '\t':9 ' ':32 '\"':34 '(':40 ')':41 ',':44
	...
	static {
		...
		ALLOW_EQUALS_IN_VALUE = Boolean.valueOf(
				System.getProperty("org.apache.tomcat.util.http.ServerCookie.ALLOW_EQUALS_IN_VALUE", "false"))
				.booleanValue();
	}

	/**
	 * Construct a new cookie collection, that will extract the information from
	 * headers.
	 * 
	 * @param headers
	 *            Cookies are lazy-evaluated and will extract the information
	 *            from the provided headers.
	 */
	...
	/**
	 * Register a new, unitialized cookie. Cookies are recycled, and most of the
	...
	public void processCookies(MimeHeaders headers) {
		...
		while (pos >= 0) {
			...
			if (cookieValue.getType() == MessageBytes.T_BYTES) {
				log.debug("Parsing b[]:{}" + cookieValue.toString());
				ByteChunk bc = cookieValue.getByteChunk();
				processCookieHeader(bc.getBytes(), bc.getOffset(), bc.getLength());
			} else {
				log.debug("Parsing S:{}" + cookieValue.toString());
	...
	public void processCookieHeader(String cookieString) {
		log.debug("Parsing cookie header {}", cookieString);
	 ...
	 * 
	 * @param value
	 *            a <code>String</code> specifying the cookie value (possibly
	 *            quoted).
	 * 
	 * @see //#setValue
	 * 
	 ...
	 * Since this is called often, this function should be organized with the
	 * most probable outcomes first.
	 */
	...
	public static final boolean isWhiteSpace(final byte c) {
		 ...
		 * '\f':; return true; default:; return false; }
		 */
		if (c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f')
			return true;
		else
	 ...
	 * [WS][$]token[WS]=[WS](token|QV)[;|,] RFC 2965 JVK
	 */
	public final void processCookieHeader(byte bytes[], int off, int len) {
		...
		while (pos < end) {
			...
			// Get the cookie broadcastName. This must be a token
			...
			// Check for an '=' -- This could also be a broadcastName-only
			// cookie at the end of the cookie header, so if we
			// are past the end of the header, but we have a broadcastName
			// skip to the broadcastName-only part.
			if (pos < end && bytes[pos] == '=') {
				...
				// token, broadcastName-only with an '=', or other (bad)
				switch (bytes[pos]) {
				case '"':
					; // Quoted Value
					isQuoted = true;
				...
				case ';':
				case ',':
					// Name-only cookie with an '=' after the broadcastName
					// token
				...
				default:
					;
					if (!isSeparator(bytes[pos]) || bytes[pos] == '=' && ALLOW_EQUALS_IN_VALUE) {
						// Token
						valueStart = pos;
						// getToken returns the position at the delimeter
						// or other non-token character
						valueEnd = getTokenEndPosition(bytes, valueStart, end, false);
						// We need pos to advance
						pos = valueEnd;
					} else {
						...
						// not valid.
						log.debug("Invalid cookie. Value not a token or quoted value.");
			...
			// We should have an avpair or broadcastName-only cookie at this
			 ...
			 * Something is wrong, but this may be a case // of having two ';'
			 * characters in a row. //
			 * log("Cookie broadcastName/value does not conform to RFC 2965");
			 * // Advance to next delimiter (ignoring everything else) while
			 * (pos < end && bytes[pos] != ';' && bytes[pos] != ',') { pos++; };
			 * pos++; // Make sure no special cookies can be attributed to //
			 * the previous cookie by setting the current cookie // to null sc =
			 * null; continue; }
			 */
			...
			if (isSpecial) {
				...
				log.debug("Unknown Special Cookie.");
	 ...
	 *             {@link #getTokenEndPosition(byte[], int, int, boolean)}
	 *             instead
	 */
	...
	/**
	 * Given the starting position of a token, this gets the end of the token,
	 * with no separator characters in between. JVK
	 */
	private static final int getTokenEndPosition(byte bytes[], int off, int end, boolean isName) {
		int pos = off;
		while (pos < end && (!isSeparator(bytes[pos]) || bytes[pos] == '=' && ALLOW_EQUALS_IN_VALUE && !isName)) {
			pos++;
		}
	...
	/**
	 * Given a starting position after an initial quote chracter, this gets the
	 * position of the end quote. This escapes anything after a '\' char JVK RFC
	 * 2616
	 */
